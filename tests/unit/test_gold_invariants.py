...
def test_bcb_olinda_expectativas_parses_mensais_ipca_components() -> None:
    cases = [
        (
...
def test_bcb_olinda_expectativas_parses_pib_despesa_adm_publica() -> None:
    body = Path(
        "tests/fixtures/official-snapshots/"
    ...
    assert quarantined == []
    assert len(silver) == 8
    assert silver[0]["seriesId"] == "PIB Despesa de consumo da administração pública"
    assert silver[0]["unit"] == "PERCENT_PER_YEAR"
    assert silver[0]["value"] == 2.2
    assert len(silver[0]["rowId"]) <= 64
    assert len({row["rowId"] for row in silver}) == 8


def test_bcb_olinda_expectativas_parses_pib_exportacao_bens_servicos() -> None:
    body = Path(
        "tests/fixtures/official-snapshots/"
    ...
    silver, quarantined = parse_bcb_olinda_expectativas(
        body,
        indicator_allowlist=["PIB Exportação de bens e serviços"],
        max_rows=8,
        indicator_units={"PIB Exportação de bens e serviços": "PERCENT_PER_YEAR"},
    )
    assert quarantined == []
    assert len(silver) == 8
    assert silver[0]["seriesId"] == "PIB Exportação de bens e serviços"
    assert silver[0]["unit"] == "PERCENT_PER_YEAR"
    assert silver[0]["value"] == 4.2676
    assert len(silver[0]["rowId"]) <= 64
    assert len({row["rowId"] for row in silver}) == 8
...
def test_bcb_olinda_expectativas_parses_producao_industrial() -> None:
    body = Path(
        "tests/fixtures/official-snapshots/"
    ...
    silver, quarantined = parse_bcb_olinda_expectativas(
        body,
        indicator_allowlist=["Produção industrial"],
        max_rows=8,
        indicator_units={"Produção industrial": "PERCENT_PER_YEAR"},
    )
    assert quarantined == []
    assert len(silver) == 8
    assert silver[0]["seriesId"] == "Produção industrial"
    assert silver[0]["unit"] == "PERCENT_PER_YEAR"
    assert silver[0]["value"] == 6.3348
    assert len(silver[0]["rowId"]) <= 64
    assert len({row["rowId"] for row in silver}) == 8


def test_bcb_olinda_expectativas_parses_ipca_15() -> None:
    body = Path(
        "tests/fixtures/official-snapshots/bcb-olinda-expectativas-ipca-15-anuais-top8.json"
    ).read_bytes()
    silver, quarantined = parse_bcb_olinda_expectativas(
        body,
        indicator_allowlist=["IPCA-15"],
        max_rows=8,
        indicator_units={"IPCA-15": "PERCENT_PER_YEAR"},
    )
    assert quarantined == []
    assert len(silver) == 8
    assert silver[0]["seriesId"] == "IPCA-15"
    assert silver[0]["unit"] == "PERCENT_PER_YEAR"
    assert silver[0]["value"] == 3.79
    assert len(silver[0]["rowId"]) <= 64
    assert len({row["rowId"] for row in silver}) == 8
