...
def test_procuradoria_panel_endpoint_is_empty_official_shell(api_client) -> None:
    response = api_client.get("/v1/procuradoria", headers=_analyst())
    assert response.status_code == 200
    body = response.json()
    assert body["commandsDisabled"] is True
    assert body["legalCommandsEnabled"] is False
    assert body["altersLegalStatus"] is False
    assert body["createsTaxCredit"] is False
    assert body["items"] == []
    assert body["queue"] == []
    assert body["g6Status"] == "LOCAL_GO_OFFICIAL_BLOCKED"


def test_procuradoria_command_is_rejected(api_client) -> None:
    response = api_client.post("/v1/procuradoria", headers=_analyst(), json={})
    assert response.status_code == 409
    detail = str(response.json().get("detail", response.json()))
    assert "desativad" in detail.lower()


def test_tech_admin_cannot_read_procuradoria_panel(api_client) -> None:
