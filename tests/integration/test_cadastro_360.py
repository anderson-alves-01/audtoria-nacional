...
def test_cadastro_360_endpoint_is_empty_shell(api_client) -> None:
    response = api_client.get("/v1/cadastro-360", headers=_analyst())
    assert response.status_code == 200
    body = response.json()
    assert body["commandsDisabled"] is True
    assert body["createsTaxCredit"] is False
    assert body["piiPresent"] is False
    assert body["territoryInvented"] is False
    assert body["minimizationEnforced"] is True
...
def test_cadastro_360_command_is_rejected(api_client) -> None:
    response = api_client.post("/v1/cadastro-360", headers=_analyst(), json={})
    assert response.status_code == 409
    detail = str(response.json().get("detail", response.json()))
    assert "desativad" in detail.lower()


def test_tech_admin_cannot_read_cadastro_360(api_client) -> None:
