...
def test_payments_panel_endpoint_is_empty_official_shell(api_client) -> None:
    response = api_client.get("/v1/payments", headers=_analyst())
    assert response.status_code == 200
    body = response.json()
    assert body["commandsDisabled"] is True
    assert body["recoveryInvented"] is False
...
def test_payments_command_is_rejected(api_client) -> None:
    response = api_client.post("/v1/payments", headers=_analyst(), json={})
    assert response.status_code == 409
    detail = str(response.json().get("detail", response.json()))
    assert "desativad" in detail.lower()


def test_tech_admin_cannot_read_payments_panel(api_client) -> None:
