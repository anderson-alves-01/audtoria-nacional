...
def test_siconfi_entes_and_ibge_pib_and_planalto_ingest(api_client) -> None:
    ...
    fundeb = api_client.post("/v1/data-sources/TESOURO-FUNDEB-VALORES/ingest", headers=_admin())
    ...
    es_frd = api_client.post("/v1/data-sources/ESTADO-ES-FRD-QUOTA/ingest", headers=_admin())
    assert es_frd.status_code == 200
    assert es_frd.json()["silverCount"] == 2
    es_compensacao = api_client.post(
        "/v1/data-sources/ESTADO-ES-COMPENSACAO-QUOTA/ingest", headers=_admin()
    )
    assert es_compensacao.status_code == 200
    assert es_compensacao.json()["silverCount"] == 2
    ...
    ms_ipi = api_client.post("/v1/data-sources/ESTADO-MS-IPI-QUOTA/ingest", headers=_admin())
    assert ms_ipi.status_code == 200
    assert ms_ipi.json()["silverCount"] == 2
    ms_cide = api_client.post("/v1/data-sources/ESTADO-MS-CIDE-QUOTA/ingest", headers=_admin())
    assert ms_cide.status_code == 200
    assert ms_cide.json()["silverCount"] == 2
    ...
    al_royalty = api_client.post(
        "/v1/data-sources/ESTADO-AL-ROYALTY-QUOTA/ingest", headers=_admin()
    )
    ...
    ma_ipi = api_client.post("/v1/data-sources/ESTADO-MA-IPI-QUOTA/ingest", headers=_admin())
    assert ma_ipi.status_code == 200
    assert ma_ipi.json()["silverCount"] == 2
    ...
    pr_ipi = api_client.post("/v1/data-sources/ESTADO-PR-IPI-QUOTA/ingest", headers=_admin())
    assert pr_ipi.status_code == 200
    assert pr_ipi.json()["silverCount"] == 2
    pr_royalty = api_client.post(
        "/v1/data-sources/ESTADO-PR-ROYALTY-QUOTA/ingest", headers=_admin()
    )
    assert pr_royalty.status_code == 200
    assert pr_royalty.json()["silverCount"] == 2
    ...
    assert by_id["ESTADO-ES-FRD-QUOTA"]["valueKind"] == "TRANSFER_AMOUNT_AS_PUBLISHED"
    assert by_id["ESTADO-ES-COMPENSACAO-QUOTA"]["valueKind"] == "TRANSFER_AMOUNT_AS_PUBLISHED"
    ...
    assert by_id["ESTADO-MS-IPI-QUOTA"]["valueKind"] == "TRANSFER_AMOUNT_AS_PUBLISHED"
    assert by_id["ESTADO-MS-CIDE-QUOTA"]["valueKind"] == "TRANSFER_AMOUNT_AS_PUBLISHED"
    ...
    assert by_id["ESTADO-AL-ROYALTY-QUOTA"]["valueKind"] == "TRANSFER_AMOUNT_AS_PUBLISHED"
    ...
    assert by_id["ESTADO-MA-IPI-QUOTA"]["valueKind"] == "TRANSFER_AMOUNT_AS_PUBLISHED"
...
def test_unavailable_sources_stay_empty(api_client) -> None:
    ...
    assert by_id["TESOURO-FUNDEB-VALORES"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["TESOURO-FUNDEB-VALORES"]["ingestAllowed"] is True
    ...
    assert by_id["ESTADO-ES-FRD-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-ES-COMPENSACAO-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-ES-ICMS-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-ES-IPI-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-ES-FRD-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-ES-COMPENSACAO-QUOTA"]["ingestAllowed"] is True
    ...
    assert by_id["ESTADO-MS-IPI-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-MS-CIDE-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-MS-ICMS-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-MS-IPI-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-MS-CIDE-QUOTA"]["ingestAllowed"] is True
    ...
    assert by_id["ESTADO-AL-ROYALTY-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    ...
    assert by_id["ESTADO-MA-IPI-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-MA-ICMS-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-MA-IPI-QUOTA"]["ingestAllowed"] is True
    ...
    assert by_id["ESTADO-PR-IPI-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-PR-ROYALTY-QUOTA"]["status"] == "TECHNICALLY_APPROVED"
    assert by_id["ESTADO-PR-ICMS-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-PR-IPI-QUOTA"]["ingestAllowed"] is True
    assert by_id["ESTADO-PR-ROYALTY-QUOTA"]["ingestAllowed"] is True
