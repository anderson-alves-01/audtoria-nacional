"""Probe BCB OLINDA for PIB Despesa de consumo das famílias (baseCalculo=1)."""
...
INDICATOR = "PIB Despesa de consumo das famílias"
...
PROBE = Path(__file__).with_name(
    "bcb-olinda-expectativas-pib-despesa-familias-anuais-top8-probe.json"
)


def main() -> None:
    encoded = quote(INDICATOR, safe="")
    filter_clause = f"Indicador%20eq%20%27{encoded}%27%20and%20baseCalculo%20eq%201"
    url = (
    ...
    assert data["value"][0]["baseCalculo"] == 1
    assert float(data["value"][0]["Mediana"]) == 1.4350
