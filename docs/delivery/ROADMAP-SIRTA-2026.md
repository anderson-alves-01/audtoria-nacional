...
| Versão da implementação local | `0.3.59` |
...
| Próximo passo automático | Preferir UF restante com CSV/API tabular recente; PA cota plena ainda DOE PDF; TO IPM/PDF; AP link SEFAZ 404; RR painel sem split ICMS/IPVA; MT XLSX ≤2015 / corrente PDF; SE/PB PROVENANCE_VERIFIED (PDF); AM HTML-per-município; SP HTML-only; SC PROVENANCE_ONLY ≤2017; PI ICMS quando Repasse WEB publicar linhas; RJ IP-blocked. ES FRD/Compensação ativados no CSV já aprovado. RFB sem carga nacional. Sem homologação humana agora. Gates G0/G1/G4/G5/G7-oficial/G8-oficial/G9/G10 permanecem BLOCKED. |
...
A implementação `0.3.59` ativa FRD e Compensação Financeira no ES (colunas `FundoReducaoDesigualdades` e `CompensacaoFinanceira` do CSV TransfEstadoMunicipios); mantém royalties AL/PR e CIDE/FEX federais via CSV COINT do Tesouro. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
