...
| Versão da implementação local | `0.3.27` |
| Linha técnica em progresso | F0/S0-S4, G6–G8 locais, Alembic, F3, onda 1 PUBLIC_OPEN, lineage Gold, 15 dashboards, FPM/RREO/DCA/RGF, observabilidade, LC 214, Portal stub, diagnóstico, regras não vinculantes, calendário catalog-official-docs-v1, shells F5–F10, upload municipal, Cadastro 360, setoriais catalogados, backfill controlado FPM/RREO/DCA, RFB territorial sem carga nacional, PE+BA+MG ICMS/IPVA TECHNICALLY_APPROVED (`state_pe_csv` / `state_ba_csv` / `state_mg_csv`) |
| Dívida Alembic `0004 -> 0005` | `RESOLVED` (`ALEMBIC_HYGIENE`) |
| Próximo passo automático | Expandir outras UFs PUBLIC_OPEN (GO/ES quando schema CSV ok); RJ IP-blocked. RFB sem carga nacional. Sem homologação humana agora. Gates G0/G1/G4/G5/G7-oficial/G8-oficial/G9/G10 permanecem BLOCKED. |
...
A implementação `0.3.27` ativa o CSV.gz Frictionless de MG (ICMS/IPVA, IBGE7 nativo) além de PE e BA, com fixture, dims município/tempo e lineage Gold. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
