...
| Versão da implementação local | `0.3.16` |
| Linha técnica em progresso | F0/S0-S4, G6–G8 locais, Alembic, F3, onda 1 PUBLIC_OPEN, lineage Gold, 15 dashboards, conectores FPM/RREO/DCA, observabilidade de ingestão, LC 214, stub Portal Transparência |
...
A implementação `0.3.16` avança observabilidade, dashboards oficiais com lineage/emptySources, LC 214 documental e stub `CREDENTIAL_REQUIRED` do Portal da Transparência. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
