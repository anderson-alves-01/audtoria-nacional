...
| Versão da implementação local | `0.3.76` |
...
A implementação `0.3.76` expande BCB OLINDA Expectativas Focus
(IPCA+Selic+Câmbio+PIB Total+PIB Serviços+PIB Agropecuária+PIB Indústria+IGP-M+IGP-DI+INPC+Dívida líquida do setor público+Dívida bruta do governo geral+Balança comercial Saldo+Resultado primário+Conta corrente+Resultado nominal+Taxa de desocupação+Investimento direto no país) além do SGS 432/433;
