...
A implementação `0.3.88` expande BCB OLINDA Expectativas Focus
(IPCA+IPCA Livres+IPCA Serviços+IPCA Administrados+IPCA Alimentação no domicílio+IPCA Bens industrializados+Selic+Câmbio+PIB Total+PIB Serviços+PIB Agropecuária+PIB Indústria+PIB Formação Bruta de Capital Fixo+PIB Despesa de consumo das famílias+PIB Despesa de consumo da administração pública+PIB Exportação de bens e serviços+PIB Importação de bens e serviços+Produção industrial+IPCA-15+IPC-Fipe+IPA-M+IPA-DI+IGP-M+IGP-DI+INPC+Dívida líquida do setor público+Dívida bruta do governo geral+Balança comercial Saldo/Exportações/Importações+Resultado primário+Conta corrente+Resultado nominal+Taxa de desocupação+Investimento direto no país) além do SGS 432/433;
