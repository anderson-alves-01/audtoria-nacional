...
A implementação `0.3.51` ativa ITR, IPI-Exportação e Royalties no CSV mensal Tesouro (mesmo arquivo PUBLIC_OPEN do FPM), e mantém as UFs estaduais já aprovadas. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
