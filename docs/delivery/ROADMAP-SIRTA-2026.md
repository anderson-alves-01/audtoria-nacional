...
A implementação `0.3.63` ativa FUNDEB por município (CSV COINT) como
`TESOURO-FUNDEB-VALORES`, distinto da complementação COUN/AJUSTE e das
retenções FPM/IPI-EXP→FUNDEB; mantém RS Compensação LC194,
AC ICMS/FUNDEB Transparência, FRD/Compensação ES, royalties AL/PR e CIDE/FEX
federais via CSV COINT. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`.
