...
| Versão da implementação local | `0.3.31` |
| Linha técnica em progresso | F0/S0-S4, G6–G8 locais, Alembic, F3, onda 1 PUBLIC_OPEN, lineage Gold, 15 dashboards, FPM/RREO/DCA/RGF, observabilidade, LC 214, Portal stub, diagnóstico, regras não vinculantes, calendário catalog-official-docs-v1, shells F5–F10, upload municipal, Cadastro 360, setoriais catalogados (ANP ativado UF=MS), backfill controlado FPM/RREO/DCA, RFB territorial sem carga nacional, PE+BA+MG+ES+MS ICMS/IPVA + GO IPVA TECHNICALLY_APPROVED (`state_pe_csv` / `state_ba_csv` / `state_mg_csv` / `state_es_csv` / `state_go_csv` / `state_ms_csv` / `anp_revendedores_api`) |
...
A implementação `0.3.31` ativa ANP revendedores (escopo UF) além de ICMS/IPVA de MS/PE/BA/MG/ES e GO IPVA. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
