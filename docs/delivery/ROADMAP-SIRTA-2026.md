...
| Versão da implementação local | `0.3.17` |
| Linha técnica em progresso | F0/S0-S4, G6–G8 locais, Alembic, F3, onda 1 PUBLIC_OPEN, lineage Gold, 15 dashboards, FPM/RREO/DCA/RGF, observabilidade, LC 214, Portal stub, diagnóstico vazio, regras não vinculantes, calendário catalog-official-docs-v1 |
...
A implementação `0.3.17` entrega shells técnicos de diagnóstico e regras de auditoria (não vinculantes), liga o calendário IBS/CBS a documentos oficiais preservados via `preservedDocuments`, e adiciona o conector SICONFI-RGF — sem tornar o catálogo operacional. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
