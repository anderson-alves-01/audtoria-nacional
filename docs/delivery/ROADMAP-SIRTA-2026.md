...
A implementação `0.3.60` ativa ICMS mensal e FUNDEB no AC (colunas `valor_icms` e `valor_fundeb` do JSON Transparência); mantém FRD/Compensação ES, royalties AL/PR e CIDE/FEX federais via CSV COINT do Tesouro. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
