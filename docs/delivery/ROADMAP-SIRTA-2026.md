...
A implementação `0.3.57` ativa IPI-Exportação/FPEX em MA (aba FPEX) e PR (coluna Fundo de Exportação), e IPI-Exportação + CIDE em MS (Tipo_Repasse no DataStore), nos arquivos PUBLIC_OPEN já usados para ICMS/IPVA; mantém CIDE/FEX federais via CSV COINT do Tesouro. Gold permanece `REAL_OFFICIAL_DATA_PENDING_HUMAN_VALIDATION`. O Programa SIRTA 2026 **não** está concluído. Pacote: `docs/delivery/HUMAN_VALIDATION_PACKAGE.md`.
